/**
 * Created by powerman on 2017/7/22.
 */
function replys_click(data,i){
    $('#reply'+data[i].c_id).click(data[i],function (event) {
        ...
        $.ajax({
            ...
            success: function (dd) {
                ...
                //添加分页按钮
                $('#huifuList'+dd[2].c_id).after(' <div class="row" id="rowHuiFu"><ul id="huifuBtnLi'+dd[2].c_id+'"></ul></div>')

                for(var r2=1;r2<=dd[0].pageBean.pages;r2++) {
                    var pack = {"pageNum":r2,"c_id":dd[0].c_id}; //包装数据
                    $('#huifuBtnLi' + dd[2].c_id).append('<li><button type="button" class="btn btn-lg btn-link btn-block" id="replyFenYeBtn' + dd[0].c_id +r2+ '">' + r2 + '</button></li>')
                    //为分页按钮添加点击事件
